...
- 在指针常量中, 指针指向的地址是不能修改的，但他所指向的内容是可以修改的, 并且在定义的时候就必须要初始化 !!  
```c++
int a = 10;
int *const p = &a;
std::cout << a <<std::endl;
*p = 30; //p指向的地址是不变的，但里面的值可以修改
std::cout << a <<std::endl;
```

### 4.2 常量指针
常量指针————指向 “常量” 的指针(const int \*p, int const \*p)  
常量指针本质上是一个指针，常量表示指针指向的内容，说明该指针指向的是一个“常量”。 
- 在常量指针中，指针自身的值是一个常量,不可改变，但可以修改其所指向的内存地址;  
```c++
    int a = 10, b = 20;
    const int *p = &a;
    std::cout << *p << std::endl;
    p = &b; //指针可以指向其他地址，*p = num 则会报错
    std::cout << *p << std::endl;
```
