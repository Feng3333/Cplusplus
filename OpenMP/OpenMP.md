...
### 使用omp后计算运行时间不能使用clock()，clock（）常见误区
- clock()计数原理是从程序开始运行到clock()函数被调用过程中，cpu的滴答数(即机器时钟每秒所走的时钟打点数)；
- 当只有一个主进程时，此时cpu滴答数是可以用来计算程序实际的运行时间的；
- 当并行多个进程同时计算时，此时cpu的滴答数是成倍增加的，此时便没有办法使用clock()来计算出实际的运行时间；
- 简单来说，clock()记录的是cpu时间，即主动在cpu上花费的时间
- gettimeofday()则是记录实际返回的挂钟时间

### 如何正确记录运行的时间？？
可以使用 openmp中的 omp_get_wtime()来记录正确的时间
```c++
omp_get_wtime();
```
